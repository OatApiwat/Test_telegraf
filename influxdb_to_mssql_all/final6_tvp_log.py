...
error_handler = logging.handlers.RotatingFileHandler(
    'error.log', maxBytes=10*1024*1024, backupCount=5  # 10MB ต่อไฟล์, เก็บไฟล์เก่าสูงสุด 5 ไฟล์
)
...
success_handler = logging.handlers.RotatingFileHandler(
    'success.log', maxBytes=10*1024*1024, backupCount=5  # 10MB ต่อไฟล์, เก็บไฟล์เก่าสูงสุด 5 ไฟล์
)
...
def get_tools_from_mssql():
    try:
        ...
        error_msg = f"[get_tools_from_mssql] Error fetching measurements from MSSQL: {str(e)}"
        print(error_msg)
        error_logger.error(error_msg)
        return []
